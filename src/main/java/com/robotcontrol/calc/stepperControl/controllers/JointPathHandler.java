...
import static com.robotcontrol.parameters.dynamic.Motion.MICROSTEPS;

class JointPathHandler {
    ...
    static SteppersPath makeStepperPath(PositionalPath positionalPath) {
        ...
        for (int i = 0; i < positionalPath.getPath().length; i++) {
            lists.add(makeSingleStepperPath(positionalPath.getPath()[i],
                    PhysicalParameters.REDUCTION_RATIO[i], MICROSTEPS[i]));
    ...
    private static List<Integer> makeSingleStepperPath(SingleJointPath path, double reductionRatio, byte microsteps) {

        List<SingleJointPoint> points = path.getJointPoints();

        double step = Math.toRadians(PhysicalParameters.STEP) / microsteps;

        ArrayList<Integer> timePath = new ArrayList<>((int) (path.getJointPoints().size() * 0.5));
