...
public class G03Handler {
    public static void calcPath(AngularGCode gCode, double startTime) throws BoundsViolation, ImpossibleToImplement {
        initialize(gCode);
     ...
     *
     */
    public static void initialize(AngularGCode gCode) {
     ...
     *
     */
    private static void makeGCodePath(AngularGCode gCode) throws
            ImpossibleToImplement,
            BoundsViolation {
    ...
    private static void incompletePath(AngularGCode gCode)
        ...
        if (gCode.getStartVelocity() != gCode.getFinalVelocity()) {
            time = Math.abs((2 * gCode.getDistance())
                    / (Math.abs(gCode.getStartVelocity()) + Math.abs(gCode.getFinalVelocity())));
            ...
            makeDynamicPath(lastTime, lastPosition, gCode.getAcceleration(),
                    maxVelocity, gCode.getFinalVelocity(), gCode);
        }
    }
    ...
    private static void makeStaticPath(double pathStartTime,
        ...
        makeStaticPathCycle(gCode, pointsNumber, currentAngle, oneSegmentAngle,
                currentPosition, pathStartTime);
